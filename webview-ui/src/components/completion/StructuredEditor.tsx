...
import { Completion } from "../../domain/Prompt";
...
function StructuredEditor({ data, output, onPromptChanged }: CompletionProps) {
  ...
  const renderExample = (i: number) => {
    return (
      <div className="horizontal-flex" key={`example-${i}`}>
        <div className="fill">
          {data.examples!.map((field, fieldIndex) => (
            <StructuredColumn
              label={field.name}
              labelEditable={false}
              isOutput={false}
  ...
  const renderTest = () => {
    return data.examples!.map((field, i) => (
      <StructuredColumn
        label={field.name}
        labelEditable={true}
  ...
  return (
    <div className="main-content">
      ...
      <Collapse
        ...
        <div className="fill">{[...Array(totalExamples)].map((x, i) => renderExample(i))}</div>
      </Collapse>
      ...
      <Collapse
        ...
        <div className="fill">{renderTest()}</div>
      </Collapse>
    </div>
  );
}
